...
function AdminDashboard({ onLogout }) {
  ...
  const formatDueDate = (dueDate) => {
    if (!dueDate) return "";
    // datetime-local gives "2030-08-06T04:06" → need "2030-08-06T04:06:00"
    if (dueDate.length === 16) return dueDate + ":00";
    return dueDate;
  };
  ...
  const handleAssignDeadline = async () => {
    try {
      ...
      const formattedDate = formatDueDate(assignForm.dueDate);
